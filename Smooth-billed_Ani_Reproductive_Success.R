...
# data prep fro reproductive success models----
...
ANI<- ANI[! (ANI$ADULTS==1), ] #removes the record of 1 adult from the datset
...
#if the group failed to produce offspring at a repvious stage, they were omitted from successive categories
...
# PCA to extrapolate ages based on morphometric data----
...
#for loop to get Jackknife Leave one out Cross-validation
library(sampling)
...
#bias corrected measures of the coeffciient and intercept 
...
# generating span and skew dataset----

#bringing in the unknown aged chicks
UNK_DATE<-read.csv("C:/Users/quinl/Desktop/ANI/METRICS_UNK_AGES.csv")
...
#rearranging data for use in analyses
#reordering
...
#bring in data on all groups with known reproductive outputs, of which we have morphometric data 
...
#new dataset with known chick ages
...
#calculating skew metric for chicks with known ages
...
#removes duplicate measures of each chick in the groups of chicks with unknown ages and keeps the first instance
SKEW_1<-setDT(UNK_DATE)[ , .SD[(!duplicated(CHICK))], by = GRP]

#reformats new df to be more similar to previous
SKEW_1<-SKEW_1%>%
  group_by(GRP)%>%
...
#calculating skew metric for chicks with extrapolated ages
...
#merges all the chicks with known ages, and chicks with extrapolated ages into one df
...
# Skew and Span modelling----

#skew and span model
skew_mod5<-glmmTMB(cbind(FLEDGED, HATCHED-FLEDGED) ~ SPAN*SKEW+ADULTS+(1|LOCATION)+(1|YEAR), 
                   data=SKEW_DATA, 
                   family = binomial(link = "logit"))
summary(skew_mod5)
...
grid.arrange(
  plot(ggpredict(skew_mod5, 
    ...
    labs(x="Hatching span (days)", y="Probability of fledging", title="a")+
    theme(
      text = element_text(family="Arial", size = 12, color="black"),
      axis.text.x = element_text(size=12, color = "black"),
      axis.text.y = element_text(size=12, color="black"),
  ...
  plot(ggpredict(skew_mod5, 
    ...
    geom_jitter(data=SKEW_DATA, aes(SKEW, FpHa), 
                ...
                pch=21, 
                size=3)+
    labs(x="Skew index", y="Probability of fledging", title="b")+
    theme(
      text = element_text(family="Arial", size = 12, color="black"),
      axis.text.x = element_text(size=12, color = "black"),
      axis.text.y = element_text(size=12, color="black"),
